...
Fixes the overestimation bias of vanilla DQN by decoupling action *selection*
(online network) from action *evaluation* (target network):

    target = r + γ · Q_target(s', argmax_a Q_online(s', a))

Reference: van Hasselt et al. (2015) — "Deep Reinforcement Learning with
...
class DoubleDQNAgent(BaseAgent):
    """Double Deep Q-Network — decoupled action selection & evaluation."""

    def __init__(self, env_info: Dict[str, Any], **kwargs):
        if torch is None:
            raise ImportError("PyTorch required for DoubleDQN.")
    ...
    def update(self, transition: Transition) -> Dict[str, float]:
        ...
        loss = F.smooth_l1_loss(q_values, targets)   # Huber loss — more robust
