...
Combines the Dueling Network architecture with Double DQN for improved
performance in environments where many actions have similar values.

Key idea: separate value V(s) and advantage A(s,a) streams let the agent
learn which states are valuable without experiencing every action in each state.

References:
  Wang et al. (2016) — "Dueling Network Architectures for Deep RL"
  https://arxiv.org/abs/1511.07401
"""
...
class DuelingDQNAgent(BaseAgent):
    """Dueling DQN with Double Q-learning target."""

    def __init__(self, env_info: Dict[str, Any], **kwargs):
        if torch is None:
            raise ImportError("PyTorch required for DuelingDQN.")
    ...
    def update(self, transition: Transition) -> Dict[str, float]:
        ...
        # Double DQN target using dueling networks
        with torch.no_grad():
    ...
    def get_analysis_data(self):
        """Return Q-value table for tabular-style analysis (discrete obs only)."""
        if not self._discrete_obs:
            return None
        with torch.no_grad():
            all_states = torch.eye(self._obs_n, dtype=torch.float32).to(self.device)
            q_table = self.q_net(all_states).cpu().numpy()
